  ...
  Record match_state  (st: DxState.state) (m: mem) : Prop :=
    {
      ...
      minvalid : ~Mem.valid_block (bpf_m st) bl_state /\ ~Mem.valid_block (bpf_m st) ins_block  (*forall chunk ofs p, ~Mem.valid_access (bpf_m st) chunk bl_state ofs p /\ ~Mem.perm (bpf_m st) bl_state ofs Cur p *)(**r ysh: bl_state doesn't exist in st *)
    }.
