...
Section Upd_flag.
  ...
  Definition match_res : res -> val -> State.state -> Memory.Mem.mem -> Prop := fun x v st m => match_state state_block st m /\ v = Vundef.
  ...
  Proof.
    ...
    simpl in c.
    assert (Hst2 := Hst).
    apply (upd_flags_store _ _ _ (int_of_flag c)) in Hst2 as Hstore.
