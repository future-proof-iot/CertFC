...
(*
Fixpoint check_mem_auxL (st: state) (perm: permission) (chunk: memory_chunk) (addr: val) (mrs: list memory_region): val :=
  match mrs with
  | [] => Vnullptr
  | hd :: tl =>
    let check_mem := check_mem_aux2P hd perm addr chunk in
      ...
      | Some res =>
        if res then
          check_mem_auxL st perm chunk addr tl
        else
          check_mem
      | None => check_mem
      end
...
Proof.
  ...
  rewrite Hptr, Hstar, Hsize in H0.
  ...
  match goal with
  | H: (if ?X then _ else _) = _ |- _ =>
    destruct X eqn: Hcmp; [| inversion H]
  end.
  simpl in H0.
  inversion H0; subst.
...
(*
Lemma mem_inv_check_mem_aux2_valid_pointer:
  forall mr p c v st v'
    (Hmem : memory_inv st)
    (Hin_mem_regions: In mr (bpf_mrs st))
    (Hcheck_mem_aux2: check_mem_aux2 mr p (Vint c) v st = Some (v', st)),
      (exists b ofs, v' = Vptr b ofs /\ Mem.valid_pointer (bpf_m st) b (Ptrofs.unsigned ofs) = true)
        \/ v' = Vnullptr.
Proof.
  intros.
  unfold memory_inv in Hmem.
  ...
  unfold check_mem_aux2 in Hcheck_mem_aux2.
  unfold is_well_chunk_bool, get_start_addr, get_block_size, get_block_perm, get_sub, get_add, rBPFAST.memory_chunk_to_valu32, rBPFValues.compu_le_32, rBPFValues.compu_lt_32, rBPFAST.memory_chunk_to_valu32_upbound, rBPFValues.comp_eq_32, rBPFValues.val32_modu, rBPFAST.memory_chunk_to_valu32, rBPFMemType.perm_ge, Val.add, Val.sub, bindM, returnM in Hcheck_mem_aux2.
  rewrite Hptr, Hstart, Hsize in Hcheck_mem_aux2.
  revert Hcheck_mem_aux2.
  destruct v; unfold Vzero; simpl.
  all: try (intro Hcheck_mem_aux2; inversion Hcheck_mem_aux2; right; reflexivity).
  all: destruct_ifN Hcond; try (unfold Ptrofs.of_int; rewrite Ptrofs.add_zero_l; intro Hcheck_mem_aux2; try inversion Hcheck_mem_aux2).
  - left.
    eexists; eexists. split; [ reflexivity |].
    rewrite Mem.valid_pointer_nonempty_perm.
    apply Mem.perm_implies with (p1 := MemRegion.block_perm mr); [ | constructor].
    ...
    apply Clt_implies_Zlt in Hcond.
    apply Cle_implies_Zle in Hcond0.
    change (Int.unsigned (Int.repr 4294967294)) with 4294967294 in Hcond0.
    change (Int.unsigned (Int.repr 1)) with 1 in Hcond.
    assert (Heq: Int.unsigned (Int.repr (Int.unsigned (Int.sub c start) + 1)) = Int.unsigned (Int.sub c start) + 1). {
      rewrite Int.unsigned_repr; [reflexivity |].
      change Int.max_unsigned with 4294967295.
      lia.
    }
    rewrite Heq in Hcond; clear Heq.
    lia.
  - intro H; inversion H; right; reflexivity.
  - left.
    eexists; eexists. split; [reflexivity |].
    rewrite Mem.valid_pointer_nonempty_perm.
    apply Mem.perm_implies with (p1 := MemRegion.block_perm mr); [ | constructor].
    ...
    apply Clt_implies_Zlt in Hcond.
    apply Cle_implies_Zle in Hcond0.
    change (Int.unsigned (Int.repr 4294967293)) with 4294967293 in Hcond0.
    change (Int.unsigned (Int.repr 2)) with 2 in Hcond.
    assert (Heq: Int.unsigned (Int.repr (Int.unsigned (Int.sub c start) + 2)) = Int.unsigned (Int.sub c start) + 2). {
      rewrite Int.unsigned_repr; [reflexivity |].
      change Int.max_unsigned with 4294967295.
      lia.
    }
  ...
  - left.
    ...
    apply Clt_implies_Zlt in Hcond.
    apply Cle_implies_Zle in Hcond0.
    change (Int.unsigned (Int.repr 4294967291)) with 4294967291 in Hcond0.
    change (Int.unsigned (Int.repr 4)) with 4 in Hcond.
    assert (Heq: Int.unsigned (Int.repr (Int.unsigned (Int.sub c start) + 4)) = Int.unsigned (Int.sub c start) + 4). {
      rewrite Int.unsigned_repr; [reflexivity |].
      change Int.max_unsigned with 4294967295.
      lia.
    }
  ...
  - left.
    ...
    apply Clt_implies_Zlt in Hcond.
    apply Cle_implies_Zle in Hcond0.
    change (Int.unsigned (Int.repr 4294967287)) with 4294967287 in Hcond0.
    change (Int.unsigned (Int.repr 8)) with 8 in Hcond.
    assert (Heq: Int.unsigned (Int.repr (Int.unsigned (Int.sub c start) + 8)) = Int.unsigned (Int.sub c start) + 8). {
      rewrite Int.unsigned_repr; [reflexivity |].
      change Int.max_unsigned with 4294967295.
      lia.
    }
...
Lemma check_mem_same_state :
  forall perm chunk addr st1
    ...
    (Hmem : memory_inv st1),
    exists ret,
      check_mem perm chunk (Vint addr) st1 = Some (ret, st1).
Proof.
  intros.
  ...
  assert (Hcheck_mem_aux: forall n p c v st mrs, exists v', check_mem_aux n p c v mrs st = Some (v', st)). {
    ...
    assert (Hcheck_mem_aux2: forall mr p c v st, exists v', check_mem_aux2 mr p c v st = Some (v', st)). {
      intros.
      unfold check_mem_aux2, is_well_chunk_bool, get_start_addr, get_block_size, get_sub, get_add, get_block_perm.
      unfold_monad.
    ...
    (**r discuss (MemRegion.MyMemRegionsIndexnat mrs n) =? default_memory_region *)
  ...
  unfold inv_memory_region in Hin_mem_regions'.
    ...
    destruct rBPFValues.cmp_ptr32_null.
    + eexists; eauto. destruct b; eauto.
  ...
  all: unfold cmp_ptr32_nullM; destruct rBPFValues.cmp_ptr32_null; eexists; reflexivity.
Qed. *)
...
Proof.
  ...
  change Vnullptr with (Vint Int.zero) in *.
  destruct cmp_ptr32_null eqn: Hcmp; [| inversion H0].
  destruct b0; [inversion H0 | ].
